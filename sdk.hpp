...
struct ScriptVariant_t {
	...
	operator int64() const					{ return static_cast<int64>(m_int); }
	operator float() const					{ return m_float; }
	operator const char* () const			{ return (m_pszString) ? m_pszString : ""; }
	operator const Vector &() const			{ static Vector vecNull(0, 0, 0); return (m_pVector) ? *m_pVector : vecNull; }
	operator char() const					{ return m_char; }
	operator bool() const					{ return m_bool; }
	operator HSCRIPT() const				{ return m_hScript; }
